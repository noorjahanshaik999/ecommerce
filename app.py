...
def cart(itemid,name,price):
    if session.get('user'):
        if request.method=='POST':
            ...
            session.modified=True
            flash(f'Item already in cart quantity increased to +{qty}')
            return redirect(url_for('viewcart'))
    return redirect(url_for('login'))
@app.route('/viewcart')
def viewcart():
...
def buyallitems():
    if session.get('user'):
        ...
        print(data.items())
        for i in data:
            k={'price_data':{'product_data':{'name': data[i][0]},'unit_amount': int(data[i][3])*100,'currency': 'inr'},'quantity':data[i][2]}
            line_items.append(k)
        #print(line_items)
        checkout_session=stripe.checkout.Session.create(
...
def allcheckout():
    if session.get('user'):
        ...
        for i in cart:
            cursor=mysql.connection.cursor()
            cursor.execute('SELECT itemid from additems where itemid=%s',[i])
            rid=cursor.fetchone()[0]
            lst.append((i,cart[i][0],cart[i][1],cart[i][1]*cart[i][2],session.get('user')))
            cursor.close()
        session[session.get('user')]={}
        session.modified=True
        cursor=mysql.connection.cursor()
        cursor.executemany('insert into orders(itemid,name,qty,total_price,username) values(%s,%s,%s,%s,%s)',lst)
        mysql.connection.commit()
...
def invoice(ordid):
    ...
    qty=orders[3]
    cost=orders[4]
    cursor.execute('select username,mobile,email,address from signup where username=%s',[username])
    data=cursor.fetchone()
    uname=data[0]
    uaddress=data[3]
    uphnumber=data[1]
    html = render_template('billdetails.html',uname=uname,uaddress=uaddress,uphnumber=uphnumber,oname=oname,qty=qty,cost=cost)
